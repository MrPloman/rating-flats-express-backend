...
export class FlatController {
    ...
    public getFlatById(req: Request, res: TypedResponse<JSONResponseInterface>) {
        if (!req.params.id) res.json(generateJsonResponse(req.method, undefined, undefined, 400, "Required valid id :: getFlatById"));
        else {
            try {
                flat.findById(req.params.id, (_err: Error, data: IFlat) => {
                    if (_err) res.json(generateJsonResponse(req.method, undefined, _err, 404, `Id "${req.params.id}" not found in Flats collection :: getFlatById`));
                    if (data) res.json(generateJsonResponse(req.method, data, undefined, 200, `Flat with ${req.params.id} found in Flats collection :: getFlatById`));
                });
            } catch (error: any) {
                if (error) res.json(generateJsonResponse(req.method, undefined, error, 500, "Something went wrong... :: getFlatById"));
            }
        }
    }
    /**
    ...
    public createFlat(req: Request, res: TypedResponse<any>) {
        if (!req || !req.body || !req.body.flat) res.json(generateJsonResponse(req.method, undefined, undefined, 400, "Required a valid body, missing flat in body :: createFlat"));
        else {
            try {
                flat.findOne({ "location.lat": req.body.flat.location.lat, "location.lng": req.body.flat.location.lng }, (_err: Error, flatFound: IFlat) => {
                    if (flatFound) res.json(generateJsonResponse(req.method, undefined, undefined, 406, "Values from location.lng and location.lat are being already used, please select other coordinates :: createFlat"));
                    else if (_err) res.json(generateJsonResponse(req.method, undefined, _err, 406, "Error in the query :: createFlat"));
                ...
                if (error) res.json(generateJsonResponse(req.method, undefined, error, 500, "Internal Server Error :: createFlat"));
            }
        }
    }
