...
export class FlatController {
        ...
        if (!req.params.id) res.json(generateJsonResponse('GET', undefined, undefined, 400, 'Required valid id'))
            ...
            try {
                flat.findById(req.params.id, async (_err: Error, data: IFlat) => {
                    ...
                    if (!!data) await res.json(generateJsonResponse('GET', data, undefined, 200, `Flat with ${req.params.id} found in Flats collection`));
                })
            } catch (error: any) {
    ...
    public createFlat(req: Request, res: TypedResponse<any>) {
        if (!req || !req.body || !req.body.flat) res.json(generateJsonResponse('POST', undefined, undefined, 400, "Required a valid body, missing flat in body"));
        else {
            try {
                flat.findOne({ "location.lat": req.body.flat.location.lat, "location.lng": req.body.flat.location.lng }, async (_err: Error, flatFound: IFlat) => {
                    if (!!flatFound) await res.json(generateJsonResponse('POST', undefined, undefined, 406, "Values from location.lng and location.lat are being already used, please select other coordinates"));
                    else if (!!_err) await res.json(generateJsonResponse('POST', undefined, _err, 406, "Error in the query"));
                    else if (!_err && !flatFound) {
                        await flat.create(req.body.flat, async (_err: Error, flatCreated: IFlat) => {
                            if (!!_err || !flatCreated) await res.json(generateJsonResponse('POST', undefined, _err, 406, "Not acceptable values in some fields of BODY"));
                            if (!!flatCreated) await res.json(generateJsonResponse('POST', { flat: flatCreated }, undefined, 201, "Accepted body values. Flat created properly"));
                        })
                    }
                })
            } catch (error) {
                if (!!error) res.json(generateJsonResponse('POST', undefined, error, 500, "Internal Server Error"));
            }
        }
    };
}
