				...
				if (ratingsFound) {
					...
					if (!!averageRating) {
						flat.findByIdAndUpdate(req.params.flatId, { rating: averageRating.rating }, { upsert: true, new: true }, (_err: any, flatUpdated: IFlat) => {
							...
							if (!!flatUpdated) res.json(generateJsonResponse(req.method, flatUpdated, undefined, 200, `Mailbox with id: "${req.params.flatId}" has been updated with average rating:: updateAverageRatingOfFlat`));
						})
					}
					else {
